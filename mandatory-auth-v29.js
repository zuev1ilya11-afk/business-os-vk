(()=>{
  ...
  function show(html){if(!gate)return;gate.innerHTML=`<div class="authGateCard">${html}</div>`;gate.style.display='flex';body.classList.remove('bos-auth-ok')}
  ...
  async function vkLogin(){
    show(`<div class="authLogo">Домашний мастер</div><h1>Вход через VK</h1><p class="muted">Подтверждаем аккаунт…</p>`);
