...
const ids=x=>[x?.id,x?.staff_id,x?.master_staff_id,x?.master_id,x?.master_vk_id,x?.vk_user_id,x?.user_id,x?.external_id].filter(Boolean).map(String);
...
function allMasters(){
  ...
  [state?.masters,state?.users,state?.staff,state?.business_staff].forEach(list=>(Array.isArray(list)?list:[]).forEach(item=>{
    if(list===state?.masters||String(item?.role||'')==='master')add(item);
  }));
...
function decorateMasterCalendar(){
  ...
  if(!root){root=document.createElement('section');root.className='card bosUnifiedMasterCard';host.prepend(root)}
  ...
  if(!root.querySelector('.bosMasterScheduleInlineHead')){
    ...
    bar.innerHTML='<div><b>Мой график</b><small>Полный день — 10:00–20:00, неполный день отмечен отдельно</small></div><button type="button" class="secondary" onclick="resetMasterCalendarMonth()">Этот месяц</button>';
    root.prepend(bar);
  }
