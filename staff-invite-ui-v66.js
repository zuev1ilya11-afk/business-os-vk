(()=>{
  ...
  function findStaff(id){const s=appState();return (s?.users||[]).find(x=>String(x.id)===String(id)||String(x.vk_user_id||x.external_id)===String(id))}

  const base=typeof openEmployeeProfile==='function'?openEmployeeProfile:null;
  if(base){
    openEmployeeProfile=function(id){
      const result=base.apply(this,arguments);
      try{
        const s=appState();
        if(String(s?.user?.role||'')!=='owner')return result;
        ...
        button.onclick=async()=>{
          ...
          try{
            ...
            section.querySelector('[data-reissue-invite]').onclick=()=>{closeModal();openEmployeeProfile(u.id)};
