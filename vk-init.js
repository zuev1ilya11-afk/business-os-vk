(function(){
  ...
  // Hold only that protected bootstrap until the auth layer has created a BOS session.
  if(typeof window.fetch==='function'&&!window.BOS_EARLY_BOOTSTRAP_GUARD){
    ...
    window.fetch=async function(input,init){
      ...
      const protectedBootstrap=url.includes(MINI_MARK)&&String(action)==='bootstrap'&&!isLocalDev()&&!headers.get('X-BOS-Session');
