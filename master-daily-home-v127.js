...
function dateLabel(o){
  ...
  const short=target.toLocaleDateString('ru-RU',{day:'2-digit',month:'2-digit'});
  if(diff===0)return`Сегодня · ${short}`;
  if(diff===1)return`Завтра · ${short}`;
  const weekday=target.toLocaleDateString('ru-RU',{weekday:'short'}).replace(/\.$/,'');
  return `${short} · ${weekday.charAt(0).toUpperCase()+weekday.slice(1)}`;
}
...
function render(){
  ...
  const orders=mine(),now=Date.now(),next=nextOrder(orders,now),attention=orders.map(o=>({o,reason:attentionReason(o,now)})).filter(x=>x.reason).sort((a,b)=>scheduleTs(a.o)-scheduleTs(b.o));
  const sig=JSON.stringify([orders.map(o=>[o.id,o.status,o.scheduled_date,o.scheduled_time,o.time_slot,o.address,o.client,o.master_called_at,o.master_agreed_at,o.master_workflow_stage,o.report_uploaded_at,o.report_act_url,o.report_review_status,o.report_review_comment]),attention.map(x=>[x.o.id,x.reason])]);
  ...
  box.dataset.sig=sig;
  box.innerHTML=`<div class="masterV127Head"><div><small>МАСТЕР</small><h2>Рабочий день</h2></div><button type="button" class="secondary" onclick="show('orders')">Все заявки</button></div>${nextHtml(next)}<div class="masterV127Attention"><div class="masterV127AttentionHead"><b>Требуют внимания</b><span>${attention.length}</span></div>${attentionHtml(attention)}</div>`;
}
