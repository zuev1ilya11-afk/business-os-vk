...
function sameDayOrders(orders,next){
  const day=dateOf(next);if(!day)return[];
  return orders.filter(active).filter(o=>String(o.id)!==String(next?.id)&&dateOf(o)===day).sort((a,b)=>scheduleTs(a)-scheduleTs(b));
}
function nextHtml(o){
...
function dayItemHtml(o){
  const time=timeOf(o)||'—',address=String(o.address||o.client_address||'Адрес не указан');
  return `<button type="button" class="masterV127DayItem" data-order-id="${escv(o.id)}" onclick="openOrder('${escv(o.id)}')"><div class="masterV127DayWhen"><b>${escv(time)}</b><span>${escv(dateLabel(o))}</span></div><div class="masterV127DayMain"><b>№ ${escv(orderNo(o))}</b><p>${escv(address)}</p></div><span class="masterV127DayArrow">›</span></button>`;
}
function dayHtml(items){
  if(!items.length)return'';
  return `<div class="masterV127Day"><div class="masterV127DayHead"><b>Остальные заявки на день</b><span>${items.length}</span></div><div class="masterV127DayRow">${items.map(dayItemHtml).join('')}</div></div>`;
}
...
function render(){
  ...
  const orders=mine(),now=Date.now(),next=nextOrder(orders,now),sameDay=sameDayOrders(orders,next),attention=orders.map(o=>({o,reason:attentionReason(o,now)})).filter(x=>x.reason).sort((a,b)=>scheduleTs(a.o)-scheduleTs(b.o));
  const sig=JSON.stringify([orders.map(o=>[o.id,o.external_id,o.status,o.scheduled_date,o.scheduled_time,o.time_slot,o.address,o.client,o.master_called_at,o.master_agreed_at,o.master_workflow_stage,o.report_uploaded_at,o.report_act_url,o.report_review_status,o.report_review_comment]),attention.map(x=>[x.o.id,x.reason])]);
...
.masterV127{margin:0 0 16px;padding:14px;border:1px solid rgba(96,165,250,.2);border-radius:18px;background:linear-gradient(180deg,rgba(37,99,235,.08),rgba(15,23,42,.16))}.masterV127Head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px}.masterV127Head small{display:block;color:var(--muted,#91a3b7);font-size:10px;font-weight:900;letter-spacing:.1em}.masterV127Head h2{margin:2px 0 0;font-size:21px}.masterV127Head button{min-height:38px}.masterV127Next{width:100%;display:grid;grid-template-columns:72px minmax(0,1fr) 18px;gap:12px;align-items:center;text-align:left;padding:14px;border:1px solid rgba(96,165,250,.28);border-radius:15px;background:rgba(37,99,235,.1);color:inherit;cursor:pointer}.masterV127When{display:flex;flex-direction:column;align-items:center;padding:8px 4px;border-radius:12px;background:rgba(255,255,255,.05)}.masterV127When b{font-size:22px;line-height:1}.masterV127When span{margin-top:5px;color:var(--muted,#91a3b7);font-size:11px}.masterV127Main{min-width:0}.masterV127Main small{color:var(--muted,#91a3b7);font-size:10px;font-weight:800}.masterV127Main h3{margin:4px 0 2px;font-size:17px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.masterV127Main p{margin:0;color:var(--muted,#91a3b7);font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.masterV127Main strong{display:inline-flex;margin-top:8px;padding:4px 8px;border-radius:999px;background:rgba(96,165,250,.14);font-size:11px}.masterV127Arrow{font-size:28px;color:var(--muted,#91a3b7)}.masterV127Empty{display:flex;flex-direction:column;gap:4px;padding:15px;border:1px dashed rgba(148,163,184,.22);border-radius:14px}.masterV127Empty span,.masterV127AttentionEmpty{color:var(--muted,#91a3b7);font-size:12px}.masterV127Day{margin-top:12px}.masterV127DayHead{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:7px}.masterV127DayHead b{font-size:13px}.masterV127DayHead span{color:var(--muted,#91a3b7);font-size:11px}.masterV127DayRow{display:flex;flex-wrap:nowrap;gap:8px;overflow-x:auto;overscroll-behavior-x:contain;scrollbar-width:thin;padding-bottom:2px}.masterV127DayItem{flex:1 0 220px;min-width:0;display:grid;grid-template-columns:62px minmax(0,1fr) 12px;gap:9px;align-items:center;padding:9px;border:1px solid rgba(96,165,250,.2);border-radius:12px;background:rgba(37,99,235,.055);color:inherit;text-align:left;cursor:pointer}.masterV127DayWhen{display:flex;flex-direction:column;align-items:center;padding:7px 3px;border-radius:10px;background:rgba(255,255,255,.045)}.masterV127DayWhen b{font-size:17px;line-height:1}.masterV127DayWhen span{margin-top:4px;color:var(--muted,#91a3b7);font-size:9px}.masterV127DayMain{min-width:0}.masterV127DayMain b{display:block;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.masterV127DayMain p{margin:3px 0 0;color:var(--muted,#91a3b7);font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.masterV127DayArrow{font-size:22px;color:var(--muted,#91a3b7)}.masterV127Attention{margin-top:12px}.masterV127AttentionHead{display:flex;align-items:center;justify-content:space-between;margin-bottom:7px}.masterV127AttentionHead b{font-size:13px}.masterV127AttentionHead span{min-width:24px;padding:2px 7px;border-radius:999px;text-align:center;background:rgba(239,68,68,.13);color:#ff9696;font-size:11px;font-weight:900}.masterV127AttentionItem{width:100%;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 11px;margin-top:6px;border:1px solid rgba(239,68,68,.18);border-radius:12px;background:rgba(239,68,68,.045);color:inherit;text-align:left;cursor:pointer}.masterV127AttentionItem>span{display:flex;min-width:0;flex-direction:column;gap:2px}.masterV127AttentionItem small{color:var(--muted,#91a3b7);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.masterV127AttentionItem strong{flex:0 0 auto;font-size:11px}.masterV127AttentionEmpty{padding:10px 0 2px}
@media(min-width:900px){.masterV127DayItem{flex-basis:0}}
@media(max-width:520px){.masterV127{padding:12px;margin-bottom:12px;border-radius:16px}.masterV127Head h2{font-size:19px}.masterV127Head button{padding:8px 10px}.masterV127Next{grid-template-columns:64px minmax(0,1fr) 12px;padding:12px 10px;gap:9px}.masterV127When b{font-size:20px}.masterV127Main h3{font-size:16px}.masterV127Main p{white-space:normal;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.masterV127DayRow{margin-right:-12px;padding-right:12px}.masterV127DayItem{flex-basis:215px}.masterV127AttentionItem{align-items:flex-start;flex-direction:column;gap:5px}.masterV127AttentionItem strong{align-self:flex-start}}
