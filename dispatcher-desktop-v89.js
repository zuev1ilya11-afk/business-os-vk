...
function ddToday(){const d=new Date();return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`}
...
function ddMasterVk(m){return String(m?.vk_user_id||'')}
function ddMasterMatches(m,o){
  const masterKeys=[m?.vk_user_id,m?.id,m?.staff_id,m?.master_staff_id].map(v=>String(v||'')).filter(Boolean);
  const orderKeys=[o?.master_vk_id,o?.master_id,o?.master_staff_id].map(v=>String(v||'')).filter(Boolean);
  return masterKeys.some(v=>orderKeys.includes(v))||String(m?.full_name||'')===String(o?.master_name||'');
}
...
function ddEnsureSelected(list){if(list.some(o=>String(o.id)===String(ddSelected)))return;ddSelected=list[0]?String(list[0].id):''}
...
function ddQueueCard(o){
  ...
  return `<button class="ddQueueCard bosFilteredOrder${selected}" data-order-id="${ddEsc(o.id)}" onclick="selectDispatcherDesktopOrder('${ddEsc(o.id)}')"><div class="ddQueueTop"><b>№ ${ddEsc(ddNo(o))}</b><span>${when}</span></div><div class="ddQueueClient"><strong>${ddEsc(o.client||'Клиент не указан')}</strong><b>${ddMoney(o.amount||0)}</b></div><div class="ddQueueWork">${ddEsc(o.work||'Работа не указана')}</div><div class="ddQueueAddress">${ddEsc(o.address||'Адрес не указан')}</div><div class="ddQueueBottom"><span>${ddEsc(o.master_name||'Мастер не назначен')}</span><div>${ddFlagHtml(o)}</div></div></button>`;
}
function ddMasterOptions(o){return `<option value="">Не назначен</option>${(state.masters||[]).map(m=>{const v=ddMasterVk(m);const selected=(v&&String(o?.master_vk_id||'')===v)||String(o?.master_name||'')===String(m.full_name||'');return `<option value="${ddEsc(v)}" ${selected?'selected':''} ${v?'':'disabled'}>${ddEsc(m.full_name||'Мастер')}</option>`}).join('')}`}
...
function ddMasterLoad(){
  ...
  const masters=(state.masters||[]).map(m=>{const assigned=active.filter(o=>ddMasterMatches(m,o));return {m,count:assigned.length,todayCount:assigned.filter(o=>ddDate(o)===today).length}}).sort((a,b)=>b.todayCount-a.todayCount||b.count-a.count||String(a.m.full_name||'').localeCompare(String(b.m.full_name||''),'ru'));
...
function ddRender(){
  ...
  return `<div class="ddDesktop"><div class="ddTop"><div><div class="ddEyebrow">ДИСПЕТЧЕРСКАЯ</div><h2>Рабочий стол</h2><span>Все заявки и мастера на одном экране</span></div><button class="primary" onclick="openOrderForm()">+ Новая заявка</button></div><div class="ddMetrics">${ddMetric('active','Активные',ddCount('active'))}${ddMetric('today','Сегодня',ddCount('today'))}${ddMetric('unassigned','Без мастера',ddCount('unassigned'),'ddMetricWarn')}${ddMetric('overdue','Просрочено',ddCount('overdue'),'ddMetricDanger')}${ddMetric('reschedule','Нужно перенести',ddCount('reschedule'),'ddMetricWarn')}${ddMetric('all','Все',ddCount('all'))}</div><div class="ddToolbar"><div class="ddSearchWrap"><span>⌕</span><input id="bosOrderSearch" value="${ddEsc(ddSearch)}" placeholder="Поиск по номеру, клиенту, телефону, адресу, работе" oninput="setDispatcherDesktopSearch(this.value)"></div><select id="bosOrderMaster" onchange="setDispatcherDesktopMaster(this.value)"><option value="">Все мастера</option>${(state.masters||[]).map(m=>`<option value="${ddEsc(m.full_name||'')}" ${ddMaster===String(m.full_name||'')?'selected':''}>${ddEsc(m.full_name||'Мастер')}</option>`).join('')}</select><button class="secondary" onclick="reloadDispatcherDesktop()">Обновить</button></div><div class="ddWorkspace"><section class="ddQueue"><div class="ddPanelTitle"><div><b>Очередь заявок</b><span id="ddQueueCount">Найдено: ${list.length}</span></div></div><div id="ddQueueList" class="ddQueueList">${list.map(ddQueueCard).join('')||'<div class="ddNothing">По выбранному фильтру заявок нет.</div>'}</div></section><div id="ddDetailRoot">${ddDetail(selected)}</div>${ddMasterLoad()}</div></div>`;
}
function ddRefreshQueue(){
  if(!ddMode()||!ddDesktop()||state.page!=='orders')return;
  const list=ddFiltered();ddEnsureSelected(list);
  const queue=document.getElementById('ddQueueList');if(queue)queue.innerHTML=list.map(ddQueueCard).join('')||'<div class="ddNothing">По выбранному фильтру заявок нет.</div>';
  const count=document.getElementById('ddQueueCount');if(count)count.textContent=`Найдено: ${list.length}`;
  const detail=document.getElementById('ddDetailRoot');if(detail)detail.innerHTML=ddDetail(ddGetSelected());
}
...
window.setDispatcherDesktopSearch=function(v){ddSearch=String(v||'');ddSelected='';ddRefreshQueue()};
window.setDispatcherDesktopMaster=function(v){ddMaster=String(v||'');ddSelected='';ddRefreshQueue()};
window.refreshDispatcherDesktopQueue=ddRefreshQueue;
