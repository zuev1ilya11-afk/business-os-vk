...
test('full Mini App UI loads and main actions work against Supabase API contract', async ({ page }) => {
  ...
  await page.route('https://obsropbslfwtanyspjbi.supabase.co/functions/v1/mini-app-api', async route => {
    ...
    let result = { ok: true };
    if (action === 'health') result={ok:true,version:'2026-09-09-sb-ui2'};
    else if (action === 'bootstrap') result = { ok:true, user:{full_name:'Илья',role:'owner',city:'Москва'}, orders:data.orders, users:data.users, masters:data.masters, sources:[{source:'VK'}], settings:{}, masterSchedule:data.masterSchedule };
