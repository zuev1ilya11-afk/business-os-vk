...
test('core UI loads and main actions work against Supabase API contract', async ({ page }) => {
  ...
  await page.route('https://obsropbslfwtanyspjbi.supabase.co/functions/v1/mini-app-api', async route => {
    ...
    if (action === 'bootstrap') {
      ...
      const order={id:'TEST-1',status:body.status||'В работе',master_name:'',master_payout:0,...body}; delete order.action; data.orders.unshift(order); result={ok:true,order};
    } else if (action === 'updateOrder') {
      const i=data.orders.findIndex(x=>String(x.id)===String(body.id)); data.orders[i]={...data.orders[i],...body}; delete data.orders[i].action; result={ok:true,order:data.orders[i]};
    } else if (action === 'addEmployee') {
      const user={vk_user_id:'staff_test',full_name:body.full_name,phone:body.phone||'',city:body.city||'Москва',role:body.role||'master',is_active:true};
      const master=user.role==='master'?{...user,specialization:body.specialization||'',work_start:body.work_start||'09:00',work_end:body.work_end||'18:00'}:null;
      data.users.push(user); if(master)data.masters.push(master); result={ok:true,user,master};
    } else if (action === 'saveMasterSchedule') {
      data.masterSchedule=(body.days||[]).map(d=>({...d,master_vk_id:body.master_vk_id||body.master_id,week_start:body.week_start})); result={ok:true,schedule:data.masterSchedule};
    }
  ...
  await expect(page.getByText(/График|Нет запланированных заявок/).first()).toBeVisible();
});