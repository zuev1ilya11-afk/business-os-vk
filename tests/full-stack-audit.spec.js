...
test('dispatcher creates and assigns order through handler, then sees assignment after reload',async({page})=>{
 ...
 await page.reload();await expect(page.locator('#authGate')).toBeHidden();await page.locator('nav [data-page=orders]').click();await page.getByRole('button',{name:'Список',exact:true}).click();const created=page.locator('.dbV94ListCard,.opsCompactOrder').filter({hasText:'Заказ аудита'});await expect(created).toHaveCount(1);await expect(created).toContainText('Тестовый мастер');
});
...
test('master calendar presets save and reload through actual handler',async({page})=>{
 const errors=[];page.on('pageerror',e=>errors.push(e.message));const {db}=await fullStack(page,'master');await page.goto('/');await expect(page.locator('#authGate')).toBeHidden();await page.locator('nav [data-page=dispatch]').click();await page.getByRole('button',{name:'Все дни',exact:true}).click();await expect(page.locator('.usDay.off')).toHaveCount(0);await page.getByRole('button',{name:'Сохранить график'}).click();await expect.poll(()=>db.tables.staff_schedule.filter(x=>x.is_working).length).toBeGreaterThan(27);await page.reload();await expect(page.locator('#authGate')).toBeHidden();await page.locator('nav [data-page=dispatch]').click();await expect(page.locator('.usDay.off')).toHaveCount(0);await page.getByRole('button',{name:'Этот месяц'}).click();expect(errors).toEqual([]);
});
