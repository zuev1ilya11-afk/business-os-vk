...
async function fullStack(page,role='owner'){
 ...
 const db=database({business_staff:[me,...(role==='master'?[]:[master])],orders:[{id:'11',client:'Анна',address:'Невский 1',work:'Монтаж',status:'В работе',amount:1000,original_amount:1000,master_staff_id:master.id,master_name:master.full_name,master_payout:297.5,source:'VK',scheduled_date:'2099-09-10'},{id:'12',client:'Борис',address:'Другой адрес',work:'Шторы',status:'В работе',amount:2000,original_amount:2000,master_staff_id:null,source:'Авито'}]});
 const handlers={};for(const s of ['mini-app-api','staff-admin-api','claims-api','profile-self-api','employee-meta-api','master-memo-api','order-meta-api','report-api'])handlers[s]=edge(s,db);
