...
test('mobile VK auth can read signed launch params from VK Bridge once',async({page})=>{
  ...
  await page.route('https://business-os-api-gateway.netlify.app/vendor/vk-bridge.js',route=>route.fulfill({status:200,contentType:'application/javascript',body:`window.vkBridge={send:async(name)=>{if(name==='VKWebAppGetLaunchParams')return {vk_app_id:'54758847',vk_user_id:'123456789',vk_language:'ru',sign:'signed_test_value'};return {}}};`}));
