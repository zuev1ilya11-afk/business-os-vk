...
async function boot(page,{orders=[],claims=[]}={}){
  await page.setContent('<main id="content"><section class="card"><div class="masterSectionTitle"><h3>Ближайшие заявки</h3></div><div class="bosMasterUpcomingDays"><div class="bosMasterUpcomingDay"><div class="bosMasterUpcomingDayHead"><b>Завтра</b><span>2099-09-10</span></div></div></div></section></main>');
...
test('claim already present as upcoming order is decorated instead of duplicated',async({page})=>{
  ...
  await page.locator('.bosMasterUpcomingDay').first().evaluate(el=>el.insertAdjacentHTML('beforeend',`<button type="button" class="bosMasterUpcomingCard" onclick="openOrder('11')"><strong class="bosUpcomingNo">№ 11</strong><span class="bosUpcomingWork">Старая работа</span><small class="bosUpcomingMeta">10:00 · 0 ₽</small></button>`));
