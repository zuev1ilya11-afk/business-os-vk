...
test('desktop dispatcher attention queue counts, marks and filters operational problems',async({page})=>{
  ...
  await page.waitForFunction(()=>window.BOS_DISPATCHER_ATTENTION_V122===true);
  await page.locator('nav [data-page=orders]').click();
  await page.getByRole('button',{name:'Список',exact:true}).click();

  const controls=page.locator('.da122Controls');
  await expect(controls).toBeVisible();
  await expect(controls.locator('[data-da122-filter="all"] b')).toHaveText('5');
  await expect(controls.locator('[data-da122-filter="overdue"] b')).toHaveText('1');
  await expect(controls.locator('[data-da122-filter="unassigned"] b')).toHaveText('1');
  await expect(controls.locator('[data-da122-filter="conflict"] b')).toHaveText('2');
  await expect(controls.locator('[data-da122-filter="reschedule"] b')).toHaveText('1');

  await expect(page.locator('.dbV94ListCard[data-order-id="11"] .da122Badge.overdue')).toHaveText('Просрочено');
  await expect(page.locator('.dbV94ListCard[data-order-id="12"] .da122Badge.unassigned')).toHaveText('Без мастера');
  await expect(page.locator('.dbV94ListCard[data-order-id="13"] .da122Badge.conflict')).toHaveText('Конфликт');
  await expect(page.locator('.dbV94ListCard[data-order-id="15"] .da122Badge.reschedule')).toHaveText('Перенос');

  const smart=page.locator('.dsd121[data-order-id="12"]');
  await expect(smart).toBeVisible();

  await controls.locator('[data-da122-filter="conflict"]').click();
  ...
  await controls.locator('[data-da122-filter="unassigned"]').click();
  ...
  await controls.locator('[data-da122-filter="all"]').click();
