...
window.openMasterOrderAgreementV179=function(id){
  ...
  form.onsubmit=async e=>{e.preventDefault();if(saving||state.busy)return;const date=String(form.elements.scheduled_date.value||''),time=String(form.elements.scheduled_time.value||'').slice(0,5),msg=form.querySelector('.bosOrderAgreementMsg');if(!date||!time){msg.textContent='Укажите дату и время';return}saving=true;state.busy=true;if(typeof setBusy==='function')setBusy(form,true);msg.textContent='Сохраняем…';try{let current=orderById(o.id);if(!current?.master_called_at){const called=await api('markCalled',o.id);mergeOrder(o.id,called.order)}const saved=await api('setAgreementSchedule',o.id,{scheduled_date:date,scheduled_time:time});mergeOrder(o.id,saved.order);closeModal();setTimeout(()=>window.openOrder?.(o.id),0)}catch(err){msg.textContent=err?.message||String(err);if(typeof setBusy==='function')setBusy(form,false)}finally{saving=false;state.busy=false}};
};
function decorate(){
