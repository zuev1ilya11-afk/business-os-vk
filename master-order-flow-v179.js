...
let saving=false,stageSaving=false,queued=false;
...
function flowMessage(id,text){const panel=document.querySelector('#modalRoot .bosMasterWorkflow');if(String(panel?.dataset?.orderId||'')!==String(id))return;const el=panel.querySelector('.bosOrderFlowMsg')||panel.querySelector('.mwv2Msg')||panel.querySelector('.bosMwMsg');if(el)el.textContent=text||''}
...
window.masterOrderFlowStageV179=async function(id,stage){
  if(!liveMaster()||stageSaving)return;const o=orderById(id),current=stageOf(o),expected=current==='assigned'?'departed':current==='departed'?'started':'';
  if(!o||!active(o)||stage!==expected)return;
  stageSaving=true;flowMessage(id,'Сохраняем этап…');
  try{const d=await api('setStage',id,{stage});mergeOrder(id,d.order);flowMessage(id,'');setTimeout(()=>window.openOrder?.(id),0)}catch(err){flowMessage(id,err?.message||String(err))}finally{stageSaving=false}
};
function scheduledActions(o){
  ...
  return `<div class="bosOrderFlowV179" data-mode="scheduled">
    ...
    <div class="bosOrderStepList">
      ${button('Выехал',stage==='assigned'&&active(o),`masterOrderFlowStageV179('${id}','departed')`,departed)}
      ${button('Начал работу',stage==='departed'&&active(o),`masterOrderFlowStageV179('${id}','started')`,started)}
    ...
    <p class="muted bosOrderFlowMsg"></p>
  </div>`;
}
function unscheduledActions(o){const id=escv(o.id);return `<div class="bosOrderFlowV179" data-mode="agreement"><div class="bosOrderFlowTitle">Этапы выполнения заявки</div><button type="button" class="primary bosOrderAgreeBtn" onclick="openMasterOrderAgreementV179('${id}')">Договориться</button><p class="muted bosOrderAgreementNote">После согласования изменения даты и времени требуют подтверждения диспетчера или руководителя.</p><p class="muted bosOrderFlowMsg"></p></div>`}
...
function decorate(){
  ...
  const panel=document.querySelector('#modalRoot .bosMasterWorkflow');if(!panel)return;
  const id=String(panel.dataset.orderId||''),o=orderById(id),actions=panel.querySelector('.mwv2Actions,.bosMwActionsV26');if(!o||!actions)return;
...
const baseOpen=window.openOrder;if(typeof baseOpen==='function')window.openOrder=function(){const out=baseOpen.apply(this,arguments);setTimeout(decorate,40);setTimeout(decorate,200);return out};
setTimeout(decorate,0);
const style=document.createElement('style');style.textContent=`
.bosMasterWorkflow .mwv2Actions:has(.bosOrderFlowV179),.bosMasterWorkflow .bosMwActionsV26:has(.bosOrderFlowV179){display:block!important}.bosOrderFlowV179{display:grid;gap:12px}.bosOrderFlowTitle{font-size:16px;font-weight:800}.bosOrderStepList{display:grid;gap:9px}.bosOrderStepBtn,.bosOrderAgreeBtn{width:100%;min-height:48px;font-weight:800}.bosOrderStepBtn:disabled{opacity:.42;cursor:default}.bosOrderStepBtn.is-done{opacity:.72;background:rgba(45,111,224,.28);border:1px solid rgba(96,165,250,.34)}.bosOrderRescheduleBox{display:grid;gap:10px;padding:12px;border:1px solid rgba(96,165,250,.22);border-radius:14px;background:rgba(15,23,42,.42)}.bosOrderRescheduleBox>div{display:grid;gap:3px}.bosOrderRescheduleBox b{font-size:14px}.bosOrderRescheduleBox span,.bosOrderAgreementNote{font-size:12px;line-height:1.4}.bosOrderRescheduleBtn{width:100%;min-height:44px}.bosOrderAgreementNote,.bosOrderFlowMsg{margin:0}.bosMasterAgreementModalBtn{display:none!important}@media(max-width:520px){.bosOrderStepBtn,.bosOrderAgreeBtn{min-height:50px;font-size:15px}}
