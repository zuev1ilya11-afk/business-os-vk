...
function syncTab(){const b=document.querySelector('.dbV24Tab');if(!b)return;const n=issues().length,cls=`dbV24Tab ${controlMode?'primary':'secondary'}`,text=`Контроль${n?` · ${n}`:''}`;if(b.className!==cls)b.className=cls;if(b.textContent!==text)b.textContent=text}
