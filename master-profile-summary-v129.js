...
function profileHtml(t){
  return `<section id="masterProfileSummaryV129" class="masterV129Panel"><div class="masterV129Metrics">${metric('В работе',String(t.active.length))}${metric('Выполнено',String(t.done.length))}${metric('Моя выплата',moneyv(t.base))}${metric('Допработы',`+ ${moneyv(t.extras)}`,'positive')}${metric('Вычеты',t.deductions?`− ${moneyv(t.deductions)}`:moneyv(0),'negative')}${metric('ЗП за неделю',moneyv(t.weekSalary),'period')}${metric('ЗП за месяц',moneyv(t.monthSalary),'period')}${metric('Общая зарплата',moneyv(t.total),'total')}</div></section>`;
}
