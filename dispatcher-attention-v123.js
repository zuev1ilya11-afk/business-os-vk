...
function decorateCard(card,flags){
  card.classList.toggle('da123Overdue',flags.includes('overdue'));
  card.classList.toggle('da123Unassigned',flags.includes('unassigned'));
  card.classList.toggle('da123Conflict',flags.includes('conflict'));
  card.classList.toggle('da123Reschedule',flags.includes('reschedule'));
  const signature=flags.join('|');
  let badges=card.querySelector(':scope > .da123Badges');
  if(!signature){badges?.remove();return}
  if(!badges){badges=document.createElement('div');badges.className='da123Badges';const top=card.querySelector(':scope > .dbV94ListTop');top?.insertAdjacentElement('afterend',badges)}
  if(badges.dataset.signature===signature)return;
  badges.dataset.signature=signature;
  badges.innerHTML=flags.map(flag=>`<span class="da123Badge ${flag}">${labelOf(flag)}</span>`).join('');
}
function controlsHtml(counts,total){
  ...
  return defs.map(([value,label,count])=>`<button type="button" class="${activeFilter===value?'primary':'secondary'}" data-da123-filter="${value}" aria-pressed="${activeFilter===value?'true':'false'}"><span>${label}</span><b>${count}</b></button>`).join('');
}
function ensureControls(list,counts,total){
  let controls=list.querySelector(':scope > .da123Controls');
  if(!controls){
    controls=document.createElement('div');controls.className='da123Controls';
...
function renderEmpty(items,visible){
  let empty=items.querySelector(':scope > .da123Empty');
  if(visible){empty?.remove();return}
  if(!empty){empty=document.createElement('div');empty.className='da123Empty';items.appendChild(empty)}
  empty.textContent=activeFilter==='all'?'По выбранным условиям заявок нет.':`Нет заявок: ${labelOf(activeFilter).toLowerCase()}.`;
}
function cleanup(root){
  root?.querySelectorAll?.('.da123Controls,.da123Empty,.da123Badges')?.forEach(node=>node.remove());
  root?.querySelectorAll?.('.dbV94ListCard[data-order-id]')?.forEach(card=>{
    card.hidden=false;
    card.classList.remove('da123Overdue','da123Unassigned','da123Conflict','da123Reschedule');
  });
  root?.querySelectorAll?.('.dsd121[data-order-id]')?.forEach(node=>{node.hidden=false});
}
function sync(){
  queued=false;
  const root=document.getElementById('content');if(!root)return;
  if(!dispatcherDesktop()||!ordersPage()){cleanup(root);return}
...
document.addEventListener('click',event=>{
  const button=event.target.closest?.('[data-da123-filter]');if(!button)return;
  event.preventDefault();event.stopPropagation();setFilter(button.dataset.da123Filter);
});
...
#content .da123Controls{display:flex;gap:6px;overflow-x:auto;padding:0 0 2px;scrollbar-width:none}#content .da123Controls::-webkit-scrollbar{display:none}#content .da123Controls button{min-height:34px;padding:6px 9px;display:inline-flex;align-items:center;gap:6px;white-space:nowrap;font-size:11px}#content .da123Controls button b{display:grid;place-items:center;min-width:20px;height:20px;padding:0 5px;border-radius:999px;background:rgba(255,255,255,.09);font-size:10px}
#content .dbV94ListCard[hidden],#content .dsd121[hidden]{display:none!important}#content .dbV94ListCard.da123Overdue{border-color:rgba(239,68,68,.62)}#content .dbV94ListCard.da123Conflict{box-shadow:inset 3px 0 0 rgba(245,158,11,.8)}#content .dbV94ListCard.da123Unassigned{box-shadow:inset 3px 0 0 rgba(96,165,250,.72)}#content .dbV94ListCard.da123Conflict.da123Unassigned{box-shadow:inset 3px 0 0 rgba(245,158,11,.8)}
#content .da123Badges{display:flex;gap:5px;flex-wrap:wrap;margin-top:1px}#content .da123Badge{display:inline-flex;align-items:center;min-height:18px;padding:2px 6px;border-radius:999px;font-size:9px;font-weight:800;line-height:1;background:rgba(255,255,255,.07);color:#cbd5e1}#content .da123Badge.overdue{background:rgba(239,68,68,.14);color:#fca5a5}#content .da123Badge.unassigned{background:rgba(59,130,246,.14);color:#93c5fd}#content .da123Badge.conflict{background:rgba(245,158,11,.15);color:#fcd34d}#content .da123Badge.reschedule{background:rgba(168,85,247,.14);color:#d8b4fe}#content .da123Empty{padding:28px 12px;text-align:center;color:var(--muted,#91a3b7);font-size:12px}
}
