...
function previewRow(o){
  const day=completedDay(o)||'—',work=String(o?.work||o?.description||o?.service_name||'').trim(),client=String(o?.client||o?.client_name||'').trim(),address=String(o?.address||'').trim(),extras=extra(o),deduct=deduction(o);
  const hasAmount=o?.amount!==undefined&&o?.amount!==null&&o?.amount!==''&&Number.isFinite(Number(o.amount));
  return `<button type="button" class="masterCabinetV141Order" data-master-cabinet-v141-order="${escv(String(o?.id||''))}"><span class="masterCabinetV141OrderMain"><b>№ ${escv(orderNo(o))}</b><small>${escv(day)}</small></span><span class="masterCabinetV141OrderText"><b>${escv(work||'Выполненная заявка')}</b>${client?`<small>${escv(client)}</small>`:''}${address?`<small>${escv(address)}</small>`:''}</span><span class="masterCabinetV141OrderMoney">${hasAmount?`<em>Стоимость ${escv(moneyv(o.amount))}</em>`:''}<em>Начисление ${escv(moneyv(payout(o)))}</em><em class="positive">Допработы + ${escv(moneyv(extras))}</em><em class="negative">Вычет − ${escv(moneyv(deduct))}${deduct?' (уже учтён)':''}</em><strong>Итого ${escv(moneyv(salary(o)))}</strong></span></button>`;
}
...
function render(){
  ...
  const rows=doneRows(),today=todayRows(),todayTotal=today.reduce((a,o)=>a+salary(o),0),sig=JSON.stringify(rows.map(o=>[o.id,o.status,o.report_review_status,completedDay(o),o.amount,o.master_payout,o.extra_work_amount,o.uncompleted_work_amount,o.work,o.description,o.service_name,o.client,o.client_name,o.address,o.external_id]));
