...
function ownerExtraMetricHtml(){const extras=state.orders.filter(o=>o.status==='Выполнена').reduce((a,o)=>a+Number(o.extra_work_amount||0),0);return `<section class="card"><div class="row"><div><h3>Допработы мастеров</h3><p class="muted">Отдельно от основной суммы заявок</p></div><strong>${money(extras)}</strong></div></section>`}

pages.home=function(){
  if(!isMasterPreview()) return ownerHomeWithExtras()+ownerExtraMetricHtml();
  const orders=ownOrders(),done=completedOwnOrders(),active=orders.filter(o=>['Назначена','В работе'].includes(o.status));
...
pages.team=function(){
  ...
  const u=previewUser,m=employeeMasterData(u),done=completedOwnOrders(),active=ownOrders().filter(o=>['Назначена','В работе'].includes(o.status));
  return `<h2>Мой профиль</h2><section class="card"><p><b>${esc(u.full_name)}</b></p><p><span class="status info">Мастер</span></p><p><b>Телефон:</b> ${esc(u.phone||'—')}</p><p><b>Город:</b> ${esc(u.city||'—')}</p>${m?`<p><b>Специализация:</b> ${esc(m.specialization||'—')}</p><p><b>График:</b> ${esc(m.work_start||'—')}–${esc(m.work_end||'—')}</p>`:''}</section><div class="grid"><div class="card metric"><span class="muted">В работе</span><strong>${active.length}</strong></div><div class="card metric"><span class="muted">Выполнено</span><strong>${done.length}</strong></div><div class="card metric"><span class="muted">Выплата</span><strong>${money(ownPayoutTotal())}</strong></div><div class="card metric"><span class="muted">Допработы</span><strong>${money(ownExtraTotal())}</strong></div></div><button class="secondary wide" onclick="exitMasterPreview()">← Вернуться в профиль владельца</button>`;
};
