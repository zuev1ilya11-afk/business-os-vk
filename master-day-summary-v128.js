...
function placeBeforeAttention(box){
  const daily=document.getElementById('masterDailyV127'),attention=daily?.querySelector('.masterV127Attention');
  if(!daily||!attention||!box)return;
  if(attention.previousElementSibling!==box)attention.insertAdjacentElement('beforebegin',box);
}
function render(){
  queued=false;
  const root=document.getElementById('content');
  if(!root||!masterMode()||String(state?.page||'')!=='home'){document.getElementById('masterDaySummaryV128')?.remove();return}
  const done=completedToday(),base=done.reduce((a,o)=>a+payout(o),0),extras=done.reduce((a,o)=>a+extra(o),0),deductions=done.reduce((a,o)=>a+num(o.uncompleted_work_amount),0),total=base+extras;
  const sig=JSON.stringify(done.map(o=>[o.id,o.status,o.completed_at,o.scheduled_date,o.amount,o.master_payout,o.extra_work_amount,o.uncompleted_work_amount,o.report_review_status,o.report_reviewed_at,o.client,o.work]));
