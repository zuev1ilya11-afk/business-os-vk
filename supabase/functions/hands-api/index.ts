...
function localStatus(o:any,prev:any){
  const s=clean(o?.status).toUpperCase();
  if(s==='CANCELLED'||s==='CANCELED')return 'Отменена';
  // Hands completion is not enough for Business OS: a local accepted report is authoritative.
  if(String(prev?.report_review_status||'')==='approved')return 'Выполнена';
  return 'В работе';
}
function externalComment(o:any){
  ...
  const works=Array.isArray(o?.works)?o.works:[],details=works.map((w:any)=>[clean(w?.name),clean(w?.description)].filter(Boolean).join(' — ')).filter(Boolean);
  if(details.length)parts.push(`Состав работ: ${details.join('; ')}`);
  return parts.join('\n');
}
async function hands(path:string,init:RequestInit={}){
  const key=Deno.env.get('HANDS_API_KEY')||'';if(!key)throw new Error('HANDS_API_KEY_NOT_CONFIGURED');
  const headers=new Headers(init.headers||{});headers.set('X-Api-Key',key);if(init.body&&!headers.has('Content-Type')&&!(init.body instanceof FormData))headers.set('Content-Type','application/json');
  const r=await fetch(`${API_BASE}${path}`,{...init,headers}),text=await r.text();let data:any={};try{data=text?JSON.parse(text):{}}catch{data={raw:text}}
  if(!r.ok)throw new Error(`HANDS_${r.status}: ${clean(data?.error||data?.detail||text||r.statusText)}`);return data;
}
async function importOrder(db:any,o:any,staffByName:Map<string,any>){
  ...
  const sched=schedule(o),specialist=clean(o?.specialist),staff=specialist?staffByName.get(specialist.toLocaleLowerCase('ru-RU')):null,amount=money(o?.price);
  const base:any={
    external_source:'hands',external_id:localExternalId(id),source:'Hands',client:clean(o?.client_name||o?.client),phone:phones(o),address:clean(o?.address),work:workText(o),
    status:localStatus(o,prev.data),amount,original_amount:amount,master_payout:staff?.id?masterPayout(amount):0,scheduled_date:sched.scheduled_date,scheduled_time:sched.scheduled_time,
    master_name:specialist||staff?.full_name||'',source_updated_at:clean(o?.updated_at||o?.creation_time)||new Date().toISOString(),updated_at:new Date().toISOString(),sync_status:'synced'
  };
  ...
  const ins={...base,comment:externalComment(o),external_source:'hands',created_by_vk_id:'hands-api'},q=await db.from('orders').insert(ins).select('id').single();if(q.error)throw q.error;return {ok:true,id:q.data.id,created:true};
}
async function syncOrders(db:any,b:any){
  const status=clean(b.status||'ACTIVE').toUpperCase();if(status&&!['ACTIVE','COMPLETE'].includes(status))throw new Error('Неверный status');
  ...
  for(let page=1;page<=maxPages;page++){
    ...
    const d=await hands(`/orders/?${q.toString()}`),rows=listFromResponse(d);pages++;if(!rows.length)break;
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    const db=createClient(Deno.env.get('SUPABASE_URL')!,Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!,{auth:{persistSession:false}}),me=await actor(db,req);if(!me)return json({ok:false,error:'Доступ не подтверждён'},401);
    ...
    if(action==='assignSpecialist'){
      const id=externalId(b.order_id);if(!id)return json({ok:false,error:'Не указан order_id'},400);const specialist=clean(b.specialist);if(!specialist)return json({ok:false,error:'Не указан специалист'},400);
      const body:any={specialist};if(clean(b.specialist_login))body.specialist_login=clean(b.specialist_login);const data=await hands(`/orders/${encodeURIComponent(id)}/specialist/`,{method:'POST',body:JSON.stringify(body)});return json({ok:true,result:data});
    }
    if(action==='sendReport'){
      const id=externalId(b.order_id);if(!id)return json({ok:false,error:'Не указан order_id'},400);const kind=clean(b.kind).toUpperCase(),outcome=clean(b.outcome).toUpperCase();if(!['AGREED','COMPLETED'].includes(kind)||!outcome)return json({ok:false,error:'Неверный отчёт'},400);
      const body:any={kind,outcome};for(const k of ['time','comment','reason','time_change_reason'])if(clean(b[k]))body[k]=clean(b[k]);const data=await hands(`/orders/${encodeURIComponent(id)}/report/`,{method:'POST',body:JSON.stringify(body)});return json({ok:true,result:data});
    }
    if(action==='uploadFile'){
      ...
      const form=new FormData();form.append('file',new Blob([bytes],{type:mime}),name);form.append('relation',clean(b.relation||'SPECIALIST_PHOTO'));const data=await hands(`/orders/${encodeURIComponent(id)}/files/`,{method:'POST',body:form});return json({ok:true,result:data});
    }
    return json({ok:false,error:'UNKNOWN_ACTION'},404);
  }catch(e){const msg=e instanceof Error?e.message:String(e),status=msg==='HANDS_API_KEY_NOT_CONFIGURED'?503:500;return json({ok:false,error:msg},status)}
});
