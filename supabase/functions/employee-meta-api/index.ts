...
const out=(x:any,s=200)=>new Response(JSON.stringify(x,(key,value)=>['password_hash','password'].includes(key)?undefined:value),{status:s,headers:{...cors,'Content-Type':'application/json'}});
...
async function sessionUid(token:string,secret:string){const p=String(token||'').split('.');if(!secret||p.length!==3||!/^[A-Za-z0-9_-]{1,128}$/.test(p[0])||!/^\d{1,12}$/.test(p[1])||Number(p[1])<=Math.floor(Date.now()/1000))return null;const e=await hmac(`${p[0]}.${p[1]}`,secret);return e===p[2]?p[0]:null}
