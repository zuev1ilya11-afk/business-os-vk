...
function desktopActionsFor(card){
  ...
  if(next?.classList?.contains('dq159DesktopActions')&&String(next.dataset.orderId)===id)return;
...
function decorateMobileCard(actions){
  const card=actions.closest('.opsCompactOrder,.dmv2Order');
  const raw=String(card?.dataset?.orderId||card?.getAttribute?.('onclick')||'');
  const id=String(card?.dataset?.orderId||raw.match(/openOrder\('([^']+)'\)/)?.[1]||'');
...
function cleanupMobile(){document.querySelectorAll('.dq159LegacyHidden').forEach(n=>n.classList.remove('dq159LegacyHidden'));document.querySelectorAll('.dq159MobileActions').forEach(n=>n.classList.remove('dq159MobileActions'))}
...
@media(min-width:${DESKTOP_MIN}px){
  ...
  .dq159DesktopActions .dq159Action{min-height:30px;padding:5px 8px;margin:0;border-radius:8px;font-size:10px;line-height:1;display:inline-flex;align-items:center;justify-content:center;text-decoration:none}
  ...
  .dbAttention .dq159DesktopActions{gap:4px;padding:6px}.dbAttention .dq159DesktopActions .dq159Action{flex:1 1 calc(50% - 4px);padding:5px 6px}
}
